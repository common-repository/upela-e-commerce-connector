...
class Order
{
	...
	protected function filtre() {
		...
		// On met une valeur absolue car les Upela veut une valeur positive pour les discount
		$this->discount = abs( filtreFloat( $this->discount ) );
		$this->fee = filtreFloat( $this->fee);
	}
	
	protected function setInfoShopperpress() {
		include_once(PLUGIN_PATH_UPELAWORDPRESS . 'functions/shopperpress/functionsShopperpress.php');
	...
	protected function setInfoShopp() {
		include_once( PLUGIN_PATH_UPELAWORDPRESS . 'functions/shopp/functionsShopp.php');
	...
	protected function setInfoWoocommerce() {
		...
		include_once( PLUGIN_PATH_UPELAWORDPRESS . '../woocommerce/woocommerce.php' );
		...
		if ( $split[0] >= 2 && $split[1] >= 1  || $split[0] >=3 ) { // for version > 2.1.2
		...
		$this->freight = getInformation( $this->row, '_order_shipping' )+((float)getInformation( $this->row, '_order_shipping_tax' )); // Shipping Fee
		...
		$this->fees = ''; // Add Fee
		$this->total = getInformation( $this->row, '_order_total' );
		
		// Ajout des coupons
		if ( getCoupons( $this->row ) != null && $show_coupon == 'show_coupon') {
			$coupons = getCoupons( $this->row );
			foreach( $coupons as $coupon ) {
				/*var_dump( $coupon );*/
		...
		foreach ($notes as $note) {
			if (getNotePrivacy($note['comment_ID'] ) == 1) {
				...
				//array_push($this->privateNotes, $note['comment_content'] . $row1['option_value']);
				if($show_private_message) {
					array_push($this->privateNotes, $note['comment_content']);
				}
			}
		}

		if($show_customer_message) {
			array_push($this->customerMessage, getOrderMessage($this->row['ID']));
		}
		//array_push($this->customerMessage, getOrderMessage($this->row['ID']));
	...
	protected function setInfoWPeCommerce() {	
		include_once( PLUGIN_PATH_UPELAWORDPRESS . 'functions/wpecommerce/functionsWPeCommerce.php');
		$this->id_order = $this->row['id'];
	...
	protected function setInfoCart66() {
		include_once( PLUGIN_PATH_UPELAWORDPRESS . 'functions/cart66/functionsCart66.php');
		$this->id_order = $this->row['id'];
	...
	protected function setInfoJigoshop() {
		...
		$this->freight = getInformation( $this->row, 'order_shipping' )+((float)getInformation( $this->row, 'order_shipping_tax' )); // Shipping Fee
